...
const AuthButton: React.FC<AuthButtonProps> = ({ className = '' }) => {
    ...
    const isAuthPage = location.pathname === LOGIN_ROUTE || location.pathname === SIGN_UP_ROUTE || location.pathname === RESET_PASSWORD_ROUTE;
            ...
            {!isAuthPage && (
                <>
                    {user.isAuth ? (
                        ...
                        >
                            <i className="fas fa-sign-out-alt auth-link__icon"></i> Выйти
                        </span>
                    ) : (
